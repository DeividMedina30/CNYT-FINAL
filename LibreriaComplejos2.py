...
def cart_a_Polares(a):
    angulo = round(math.atan(a[1]/a[0]),2)
...
def matriz_Transpuesta(matriz1):
    matrizTranspuesta = [[[0,0] for j in range(len(matriz1))] for i in range(len(matriz1))]
    for i in range(len(matriz1)):
        for j in range(len(matriz1)):
            if i != j:
                matrizTranspuesta[i][j][0] = matriz1[j][i][0]
                matrizTranspuesta[i][j][1] = matriz1[j][i][1]
            else:
                matrizTranspuesta[i][j][0] = matriz1[i][j][0]
                matrizTranspuesta[i][j][1] = matriz1[i][j][1]
...
def leer_Matrices_Escalar(m1,c):
    if len(m1) <= 0:
        ...
        return matrizEscalar 
def leer_Matrices_Transpuesta(m1,valor):
    if len(m1) <= 0:
        print("No se puede realizar la operación.")
    else:
        if valor == 1:
            matrizTranspuesta = matriz_Transpuesta(m1)
            prettyprintingsMatrices(matrizTranspuesta)
            return matrizTranspuesta
        else:
            prettyprintingsVectores(m1,valor)
            return m1 
...
def leer_producto_Tensor_Matrices(m1,m2):
    if len(v1) == 0 or len(v2) == 0:
        ...
        prettyprintingsTensor(productoTensor,n)
        return productoTensor
def simulacion():
    x = [[(0,0),(1,0)],[(1,0),(0,0)]]
...
m3 = [[(1,1),(0,0)],[(0,0),(1,-1)]]
#leer_producto_Tensor_Matrices(m1,m2)
#leer_producto_Tensor_Vector(v1,v2)
#leer_matriz_Hermitiana(m1)
#leer_matriz_Unitaria(m1)
#leer_distancia_vectores(v1,v2)
#leer_vector_norma(v1)
#leer_Producto_Interno(v1,v2)
#leer_accion_matriz_vector(m1,v2)
#leer_Matrices_producto(m1,m2)
#leer_Matrices_Adjunta(v1,valor)
#leer_Matrices_Conjugada(v1,valor)  
#leer_Matrices_Transpuesta(m1,valor)
#leer_Matrices_Escalar(m1,c)
#leer_Matrices_Inversa(m1)
#leer_Matrices_Suma(m1,m2)
